...
<head>
    ...
    <title>Ventas &mdash; Estratega Contable</title>
        ...
        <form class="form-inline" method="post" name="xmlForm" id="xmlForm" enctype="multipart/form-data">
            ...
            <div class="col-xs-6 col-xs-offset-4">
                <div class="form-group">
                    <label for="input_factura">Venta Electrónica a subir:</label>
                    <input type="file" class="form-control" name="input_factura" id="input_factura"  required>
                </div>
                <div class="form-group">
                    <button type="submit" class="btn btn-success">Extraer y Guardar</button>
...
<script type="text/javascript">
    $('#xmlForm').submit(function (e){
        ...
        $.ajax({
            url: "App/Http/Controllers/Backend/FacturaController/store",
            type: "post",
            data: Form,
            processData: false,
            contentType: false,
    ...
    <div class="col-lg-7 col-md-12 col-12 col-sm-12">
        <div class="card">
            <div class="card-header">
                <h4>Listado de Facturas Electrónicas de Venta:</h4>
            </div>
        </div>
            ...
            <div class="table-responsive">
                <table class="table table-striped mb-0">
                    ...
                    @foreach($facturas as $factura)
                        ...
                        <tr>
                            <th>{{$factura->cliente_id}}</th>
                            <th>{{$factura->cliente}}</th>
                            <th>{{$factura->fechaEmision}}</th>
                            <th>{{$factura->secuencial}}</th>
                            <th>{{$factura->totalSinImpuestos}}</th>
                            <th>{{$factura->totalDescuento}}</th>
                            <th>{{$factura->valorICE}}</th>
                            <th>{{$factura->subtotal0}}</th>
                            <th>{{$factura->subtotal12}}</th>
                            <th>{{$factura->iva12}}</th>
                            <th>{{$factura->propina}}</th>
                            <th>{{$factura->valorTotal}}</th>
